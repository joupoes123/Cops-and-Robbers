...
--[[
        ...
        print(string.format('[CNR_CD_TRACE_ASSault] IsPedPerformingMeleeAction: %s', tostring(IsPedPerformingMeleeAction(playerPed))))
        if IsPedInMeleeCombat(playerPed) and (GetGameTimer() - lastAssaultReportTime) > assaultReportCooldown then
            ...
            -- [ Existing print for IsPedPerformingMeleeAction should be here ]
            -- print(string.format('[CNR_CD_TRACE_ASSault] IsPedPerformingMeleeAction: %s', tostring(IsPedPerformingMeleeAction(playerPed))))
            -- print('[CNR_CD_TRACE_ASSault] Melee combat and cooldown conditions met.') -- This log can be kept or adapted

            local playerCoords = GetEntityCoords(playerPed)
            local forwardVector = GetEntityForwardVector(playerPed)
            local rayStart = playerCoords + forwardVector * 0.5 -- Start ray slightly in front
            rayStart = vector3(rayStart.x, rayStart.y, rayStart.z + 0.5) -- Adjust Z for body height
            local rayEnd = playerCoords + forwardVector * 2.0 -- Ray length of 2.0 units
            rayEnd = vector3(rayEnd.x, rayEnd.y, rayEnd.z + 0.5) -- Adjust Z for body height

            -- Flag 2 means "consider peds". Flag 10 means "consider vehicles and peds". We'll use 2 for peds.
            -- Last parameter 7 means ignore player ped and objects attached to them, also ignore things player is carrying
            -- For more precise flags, one might use: entityTypes = 2 (peds), ignoredEntities = {playerPed}
            ...
            -- For debugging ray:
            -- Line(rayStart, rayEnd, 255,0,0,255) -- Draw a red line for the ray

            local rayHandle = StartShapeTestRay(rayStart.x, rayStart.y, rayStart.z, rayEnd.x, rayEnd.y, rayEnd.z, 2, playerPed, 0)
            -- Wait for the raycast to complete (usually very fast, but a small wait can be robust)
            -- A loop isn't strictly necessary here if we just check once per game tick the assault logic runs.
            -- GetShapeTestResult can be called immediately.
            ...
            if hit and entityHit and entityHit ~= 0 then
                ...
                local targetPed = entityHit -- Use entityHit as potential targetPed
                ...
                local damagedByPlayer = HasEntityBeenDamagedBy(targetPed, playerPed, true)

                print(string.format('[CNR_CD_TRACE_ASSault_TARGET_CHECKS_Ray] targetPed: %s, IsPed: %s, IsNotPlayerSelf: %s, IsNotOtherPlayer: %s, IsHuman: %s, DamagedByPlayer: %s', 
                    targetPed, tostring(isPed), tostring(isNotPlayer), tostring(isNotOtherPlayer), tostring(isHuman), tostring(damagedByPlayer)))
                ...
                if isPed and isNotPlayer and isNotOtherPlayer and isHuman and damagedByPlayer then
                    ...
                    if targetModel ~= GetHashKey("s_m_y_cop_01") and targetModel ~= GetHashKey("s_f_y_cop_01") and targetModel ~= GetHashKey("s_m_y_swat_01") and targetRelGroup ~= GetHashKey("COP") then
                        ...
                        ClearEntityLastDamageEntity(targetPed) -- Clear damage flag for this ped
                    else
                        print(string.format('[CNR_CD_TRACE_ASSault_Ray] Target ped %s is a cop model or in COP group. No crime reported.', targetPed))
                        ClearEntityLastDamageEntity(targetPed) -- Still clear damage flag to avoid re-triggering on same damage event
                    end
                elseif damagedByPlayer then
                    -- If it was damaged by player but failed other checks (e.g. was another player, or not a ped)
